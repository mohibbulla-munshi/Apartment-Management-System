...
<div class="body flex-grow-1 px-3">
  <div class="container-lg">
    <!-- Breadcrumbs -->
    <span>
      {{ Breadcrumbs::render('addTenantNotice') }}
    </span>
    <!-- User Profile info entry form -->
    <div class="card shadow p-3 mb-5 bg-body rounded">
      <div class="card-body">
        <form method="POST" action="{{ route('tenant.store') }}" class="row g-3 needs-validation" novalidate>
          @csrf
...
<div class="body flex-grow-1 px-3">
  <div class="container-lg">
    ...
    <table class="table table-striped shadow p-3 mb-5 bg-body rounded" style="width:100%">
      ...
      <tbody>
        @foreach ($tenantNotices as $tenantNotice)
        <tr>
          <td>{{ $tenantNotice->title }}</td>
          <td>{{ $tenantNotice->status }}</td>
          <td>{{ \Carbon\Carbon::parse($tenantNotice->meeting_date)->format('F j, Y') }}</td>
          <td>
            <a href="{{ url('meeting/'.$tenantNotice->id.'/edit') }}" class="btn btn btn-success rounded-0 float-end"
              type="submit">Edit</a>

            <form method="POST" action="{{ url('meeting/'.$tenantNotice->id) }}" class="float-end">
              @csrf
              @method('DELETE')
