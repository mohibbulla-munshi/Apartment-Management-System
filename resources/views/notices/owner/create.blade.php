...
<div class="body flex-grow-1 px-3">
  <div class="container-lg">
    <!-- Breadcrumbs -->
    <span>
      {{ Breadcrumbs::render('addOwnerNotice') }}
    </span>
    <!-- User Profile info entry form -->
    <div class="card shadow p-3 mb-5 bg-body rounded">
      <div class="card-body">
        <form method="POST" action="{{ route('owner.store') }}" class="row g-3 needs-validation" novalidate>
          @csrf
...
<div class="body flex-grow-1 px-3">
  <div class="container-lg">
    ...
    <span class="text-center">
      {{ Breadcrumbs::render('ownerNotice') }}
    </span>
    <!-- Own Working Space -->
    <table class="table table-striped shadow p-3 mb-5 bg-body rounded" style="width:100%">
      ...
      <tbody>
        @foreach ($ownerNotices as $ownerNotice)
        <tr>
          <td>{{ $ownerNotice->title }}</td>
          <td>{{ $ownerNotice->status }}</td>
          <td>{{ \Carbon\Carbon::parse($ownerNotice->meeting_date)->format('F j, Y') }}</td>
          <td>
            <a href="{{ url('meeting/'.$ownerNotice->id.'/edit') }}" class="btn btn btn-success rounded-0 float-end"
              type="submit">Edit</a>

            <form method="POST" action="{{ url('meeting/'.$ownerNotice->id) }}" class="float-end">
              @csrf
              @method('DELETE')
